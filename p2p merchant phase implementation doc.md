...
## 2026-08-02 Implementation Update 3 (Added)

This section records completion of the remaining end-to-end gaps from Update 2. Original content above remains unchanged.

### Completed in this pass

1. Full fiat-intent wiring across remaining purchase entry points:
   - Added a dedicated user route and page for remaining fiat purchase surfaces:
     - `platform/src/features/dashboard/pages/UserOrdersPage.tsx`
     - Route wired in `platform/src/app/router/index.tsx`
     - Navigation wired in `platform/src/app/layouts/AppLayout.tsx`
   - Implemented end-to-end intent launch for:
     - wallet funding,
     - membership fee settlement,
     - premium feature purchases,
     - promotional purchases.
   - Added fee/provider preview and intent creation using the Phase 0/1 P2P orchestration path.
   - Added `startFiatPurchase` helper in `platform/src/services/api/p2pMerchant.ts` to create a fiat intent and auto-create a P2P order when routed to the P2P provider.

2. Dedicated admin merchant wallet top-up/withdraw UI:
   - Added admin operation form to:
     - `platform/src/features/admin/pages/P2PMerchantControlPage.tsx`
   - Added RPC wrapper:
     - `platform/src/services/api/p2pAdmin.ts` (`applyMerchantWalletOperation`)
   - Admin can now execute merchant wallet top-up and withdrawal actions from the P2P control plane.

3. Full customer-side P2P order action page parity:
   - Added customer-facing P2P order page in:
     - `platform/src/features/dashboard/pages/UserOrdersPage.tsx`
   - Added direct customer actions for:
     - `I Have Paid` (payment proof submission + state transition)
     - `Request Review` (dispute open + review transition)
   - Added recent fiat intent log and active customer order management in the same page.

4. External AML/sanctions provider connector:
   - Added normalized connector flow in:
     - `platform/src/services/api/p2pCompliance.ts` (`runExternalAmlScreening`)
   - Connector supports:
     - optional external provider URL,
     - deterministic mock fallback when provider URL is absent or mock mode is enabled,
     - persistence of screening results into `p2p_risk_signals`.
   - Added operator control surface in:
     - `platform/src/features/admin/pages/P2PMerchantControlPage.tsx`

5. Merchant analytics export actions from merchant dashboard:
   - Added merchant analytics snapshot reader in:
     - `platform/src/services/api/p2pMerchant.ts` (`listMerchantAnalytics`)
   - Added CSV and Excel export actions to:
     - `platform/src/features/dashboard/pages/MerchantDashboardPage.tsx`
   - Merchant dashboard now exposes export-ready daily KPI snapshots.

### Additional support changes

- Added user-scoped membership fee invoice reader:
  - `platform/src/services/api/membershipAdmin.ts` (`listMembershipFeeInvoicesForUser`)
- Added dashboard shortcut to the new orders/payments surface:
  - `platform/src/features/dashboard/pages/DashboardPage.tsx`

### Updated closure status of Update 2 items

The following Update 2 items are now closed in code:

1. Full user purchase-surface coverage for fiat intents.
2. Merchant wallet top-up/withdraw dedicated admin UX.
3. Customer-side P2P order action UI parity.
4. External AML/sanctions provider connector.
5. Merchant self-export actions for analytics snapshots.

### Validation performed in this pass

- TypeScript validation executed after service-layer changes: `npm run typecheck` (success).
- TypeScript validation executed after user orders/payments route integration: `npm run typecheck` (success).
- TypeScript validation executed after merchant/admin UI completion: `npm run typecheck` (success).
