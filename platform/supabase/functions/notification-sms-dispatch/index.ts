...
const smsEndpoint = Deno.env.get('SMS_DISPATCH_ENDPOINT') ?? '';
const smsApiKey = Deno.env.get('SMS_DISPATCH_API_KEY') ?? '';
...
Deno.serve(async (request) => {
  ...
  if (!smsEndpoint) {
    return jsonResponse(500, { error: 'SMS_DISPATCH_ENDPOINT is not configured.' });
  }
  ...
  if (roleResult.error || roleResult.data?.role !== 'super_admin') {
    return jsonResponse(403, { error: 'Only super admins can dispatch SMS/WhatsApp/Telegram notifications.' });
  }
  ...
  const providerResponse = await fetch(smsEndpoint, {
    method: 'POST',
    headers: {
  ...
  if (!providerResponse.ok) {
    ...
    return jsonResponse(providerResponse.status, {
      error: body || 'SMS provider rejected request.',
    });
  }
